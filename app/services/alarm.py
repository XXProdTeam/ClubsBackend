...
class NotificationService:
    ...
    async def register_event(self, db: AsyncSession, user_id: int, event_id: int):
    ...
    async def unregister_event(self, db: AsyncSession, user_id: int, event_id: int):
        user = await self.user_crud.get_user_by_id(db=db, user_id=user_id)
        event = await self.event_crud.get_event_by_id(db=db, event_id=event_id)
    ...
    async def one_day_before_event(self, db: AsyncSession, user_id: int, event_id: int):
        user = await self.user_crud.get_user_by_id(db=db, user_id=user_id)
        event = await self.event_crud.get_event_by_id(db=db, event_id=event_id)
